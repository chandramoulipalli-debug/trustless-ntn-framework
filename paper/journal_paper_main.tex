...
(5 Monte-Carlo runs each, otherwise identical to Exp.~1 parameters).

\begin{table}[t]
\centering
\caption{Exp.~10: Random-graph vs 3GPP NR-NTN channel model
         (5 runs each, $N\!=\!1{,}051$, $f\!=\!20\%$, $\theta^*\!=\!0.20$)}
\label{tab:exp10}
\renewcommand{\arraystretch}{1.1}
\small
\begin{tabular}{lrrrrr}
\toprule
Channel Model & Latency (rounds) & F1 & Recall & FPR & Handovers/run \\
\midrule
Random graph  & $127.0\pm10.1$ & 0.233 & 0.132 & 0.000 & 0 \\
3GPP NR-NTN   & $86.1\pm25.9$  & 0.066 & 0.034 & 0.000 & 307{,}215 \\
\bottomrule
\end{tabular}
{\footnotesize Ka-band FSPL (20\,GHz) and Keplerian orbital mechanics (LEO 550\,km,
$T_{\rm orb}$\,=\,95.5\,min; MEO 8{,}000\,km; GEO 35{,}786\,km).
The 307{,}215 handovers/run reflect 58.4\% of LEO interactions where the
primary satellite has orbited below the 10$^\circ$ minimum elevation angle
within the 500-round (8.3\,h) simulation window; most alternate-satellite
searches also fail (orbit depleted), so the trust engine withholds those
updates rather than guessing.  FPR\,=\,0.000 is maintained in all 5 runs,
confirming the framework makes no false accusations under severe link dropout.}
\end{table}

The central finding of Exp.~10 is that \emph{FPR\,=\,0.000 is preserved in all
five runs}, even when 58.4\% of scheduled LEO observations are withheld because
the satellite has orbited below the 10$^\circ$ 3GPP elevation threshold.
The delay-aware aging formula (Eq.~\eqref{eq:aging}) contributes here in two
complementary ways.
First, the term $\Delta t\!=\!T_{\mathrm{round}} + d_{ij}/1{,}000$ absorbs the
Keplerian slant-range variation (270\,ms GEO, 5--14\,ms LEO) without requiring
protocol changes.
Second, \emph{withheld observations leave trust unchanged}: because the update
rule fires only on an observed interaction, a satellite that has orbited out of
coverage does not generate a spurious quality signal that could push an honest
node below $\theta^*$.
The reduced recall (3.4\%) is a direct consequence of the observation gap
rather than algorithm failure; nodes that \emph{are} reachable via a visible
satellite are still detected at latency comparable to the random-graph baseline
(86.1 rounds for the detectable subset).
This behavior---conservative silence under link dropout, zero false
accusations---is the safety property most relevant to the orbital-dynamics
scenario raised by reviewer MW3.
...
Technical Report (TR) 38.811, 2020, Release 15.

\bibitem{3gpp38821}
3GPP, ``Solutions for NR to Support Non-Terrestrial Networks (NTN),''
3rd Generation Partnership Project (3GPP),
Technical Report (TR) 38.821, 2021, Release 16.
