...
threshold-specific regularization effect.
At $\theta\!=\!0.40$, a fraction of legitimate Ground nodes with
$T^*_{\mathrm{legit}}\!=\!0.356$ reside near the boundary and are
incorrectly flagged (contributing to FPR\,=\,0.265 in Table~\ref{tab:comparison}).
The Gaussian DP perturbation stochastically shifts these borderline
legitimate nodes across the threshold; since legitimate trust
is distributed above the malicious cluster at every segment
($T^*_{\mathrm{legit}}\!>\!T^*_{\mathrm{mal}}$ for all $s$),
the noise net reduces false positives more than it increases them
at this specific $\theta$.
This effect is threshold-dependent: at $\theta^*\!=\!0.20$ (below
all legitimate steady-state values), FPR is already zero and DP noise
cannot further improve Precision.
