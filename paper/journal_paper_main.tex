...
Recall is bounded at $\approx\!0.14$ due to on-off adversarial evasion:
...
constrain damage within each burst window independently of the trust
score.
Detection latency (35.9\,rounds) measures how quickly the burst onset
is caught, not the fraction of malicious nodes visible at any snapshot.
All classification metrics are computed from a single aggregator node
(node~0); a multi-aggregator consensus design would reduce detection
latency but requires additional communication overhead modeling.
...
The Paillier latency benchmark uses single-threaded Python; a
hardware-accelerated implementation would reduce overhead by
10--50$\times$, extending the 2048-bit feasibility boundary
well beyond $n\!=\!75$.
...
Across nine experiments on a 1{,}000-node heterogeneous NTN
...
budget for $n\!\leq\!75$ contributors (13.9\,\% of round budget at
the extrapolated feasibility boundary).
