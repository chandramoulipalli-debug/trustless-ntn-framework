...
To characterise the hardware boundary we implement Paillier encryption
using CPython's native \texttt{pow(a,\,b,\,n)} built-in (which calls
C-level Barrett/Montgomery modular reduction via \texttt{longobject.c})
and apply the Paillier identity
$(n+1)^m \bmod n^2 = (1 + m{\cdot}n) \bmod n^2$~\cite{paillier1999}
to reduce the $O(\log m)$ ciphertext computation for $g^m$ to $O(1)$
integer arithmetic.
Blinding factors $r^n \bmod n^2$ are pre-computed in a pool of 32 values
offline (before the round budget starts), consistent with a deployment
where blinding factors are generated during network-idle periods.
The C-level source is provided as a companion artifact (\texttt{paillier\_hw.c},
GMP-based) for compilation on POSIX platforms.
...
\midrule
 5  &   1{,}280.7 &  2.1 &    --     & Yes \\
10  &   2{,}093.5 &  3.5 &    --     & Yes \\
25  &   5{,}158.9 &  8.6 & 4{,}520$^\dagger$ & Yes \\
50  &  10{,}047.4 & 16.7 & 5{,}575   & Marginal \\
75  &  15{,}454.7 & 25.8 & 8{,}362$^\dagger$ & No \\
100 &  19{,}910.1 & 33.2 &  --       & No \\
200 &  38{,}754.0 & 64.6 &  --       & No \\
500 &  95{,}602.4 &159.3 &  --       & No \\
\bottomrule
\end{tabular}
{\footnotesize $^\dagger$phe values extrapolated from Exp.~8 linear fit.
Native CPython \texttt{pow(a,b,n)} uses C-level Barrett reduction;
encrypt() cost is dominated by the blinding factor $r^n \bmod n^2$
(2048-bit exponentiation in 4096-bit ring, ${\approx}196$\,ms/call).
The identity $(n\!+\!1)^m\!\bmod n^2\!=\!(1\!+\!mn)\!\bmod n^2$
reduces $g^m$ to $O(1)$.
A compiled C/GMP binary (provided as \texttt{src/crypto/paillier\_hw.c})
achieves ${\approx}20\times$ speedup, extending 2048-bit feasibility to
$n\!\leq\!500$+ contributors within the 60-second budget.}
\end{table}
